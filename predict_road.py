...
def predict_road(path:str):
    ...
    # plt.scatter([imshape[1]*1/6],[imshape[0]/2])
    # plt.scatter([imshape[1]*5/6],[imshape[0]/2])
    ...
    vertices = np.array(
        [
            [
                (0, imshape[0]),
                (imshape[1] * 1 / 6, imshape[0] / 4),
                (imshape[1] * 5 / 6, imshape[0] / 4),
                (imshape[1], imshape[0]),
            ]
        ],
        dtype=np.int32,
    ...
    # Rango de pendiente deseado
    min_slope = 1.5  # Pendiente mínima
    max_slope = 10.0  # Pendiente máxima

    lines = []

    if linesAll is not None:
        for line in linesAll:
            x1, y1, x2, y2 = line[0]

            # Evitar división por cero
            if x2 - x1 == 0:
                continue  # Ignorar líneas verticales

            slope = (y2 - y1) / (x2 - x1)

            # Filtrar según el rango de pendientes
    ...
    if lines is not None and len(lines) > 2:
        ...
        for currentLine in lines:
            ...
            for x1, y1, x2, y2 in currentLine:
                lineLength = ((x2 - x1) ** 2 + (y2 - y1) ** 2) ** 0.5  # get line length
        ...
        if not addedPos:
            print("Not enough lines found POS")
        if not addedNeg:
            print("Not enough lines found NEG")
        ...
        for slope in positiveSlopes:
            ...
            if abs(slope - posSlopeMedian) < posSlopeMedian * 0.2:
        ...
        xInterceptPosMean = np.mean(
            np.asarray(xIntPosGood)
        )  # average of good x intercepts for positive line
    ...
    y1 = 0
    y2 = imshape[0] - imshape[0] / 4  # pollo
    x2 = (y2 - y1) / slope + x1
    ...
    y2 = int(round(y2))
    cv2.line(
        laneLines, (x1, im.shape[0] - y1), (x2, imshape[0] - y2), (255, 255, 0), 2
    )  # plot line
    cv2.line(
        colorLines, (x1, im.shape[0] - y1), (x2, imshape[0] - y2), (0, 255, 0), 4
    )  # plot line on color image
    ...
    cv2.line(
        colorLines, (x1N, im.shape[0] - y1N), (x2N, imshape[0] - y2), (0, 255, 0), 4
    )  # plot line on color iamge
    ...
    opacity = 0.25
    blendedIm = cv2.addWeighted(laneFill, opacity, im.copy(), 1 - opacity, 0)
    cv2.line(
        blendedIm, (x1, im.shape[0] - y1), (x2, imshape[0] - y2), (0, 255, 0), 4
    )  # plot line on color image
    cv2.line(
        blendedIm, (x1N, im.shape[0] - y1N), (x2N, imshape[0] - y2), (0, 255, 0), 4
    )  # plot line on color image
    ...
    finalMask = np.zeros_like(im, dtype=np.uint8)

    # Dibujar el polígono en blanco en la máscara
    cv2.fillPoly(finalMask, vertices, (255, 255, 255))

    # Aplicar la máscara sobre la imagen original
    finalMaskedIm = cv2.bitwise_and(im, finalMask)

    # Mostrar la máscara (polígono blanco y fondo negro)
    # plt.figure(11)
    # plt.imshow(finalMask, cmap='gray')
    # plt.title('Polygon mask')

    # Mostrar la imagen con solo el polígono visible
    # plt.figure(12)
    # plt.imshow(finalMaskedIm)
    # plt.title('Image with Visible Polygon')


    #cv2.imwrite("./data/images/road3.jpg", finalMaskedIm)
    cv2.imwrite("./data/images/finalMask.jpg", finalMask)
...
#predict_road("./data/images/road2.jpg")